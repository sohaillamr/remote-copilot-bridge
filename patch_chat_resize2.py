...
pattern = r'(<textarea\s+ref=\{inputRef\}\s+value=\{input\}\s+onChange=\{\(e\) => setInput\(e\.target\.value\.slice\(0, MAX_PROMPT_LENGTH\)\)\}\s+onKeyDown=\{handleKeyDown\})'
...
if re.search(pattern, content):
    ...
    with open('web/src/pages/app/Chat.tsx', 'w', encoding='utf-8') as f:
        f.write(content)
    print("Chat scaling patch applied!")
else:
    print("Failed to find textarea pattern")

# Add audit markdown
audit_text = """# Synapse Platform - Heavy UX & Functionality Audit
